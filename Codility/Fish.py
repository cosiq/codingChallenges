...
# All its elements are unique. Array B contains the directions of the fish. It contains only 0s 
# and/or 1s, where:

# 0 represents a fish flowing upstream,
# 1 represents a fish flowing downstream.
# If two fish move in opposite directions and there are no other (living) fish between them, 
# they will eventually meet each other. Then only one fish can stay alive − the larger fish eats
# the smaller one. More precisely, we say that two fish P and Q meet each other 
...
# Write a function that, given two non-empty arrays A and B consisting of N integers,
# returns the number of fish that will stay alive.

def fish(A, B):
    stack, res = [], 0
    for i in range(0, len(A)):
        if B[i] == 1: stack.append(A[i])
        else:
